...
fun PopularMoviesScreen(
    ...
    if(movieLisState.popularMovieList.isEmpty()){
        Box(
        ...
        LazyVerticalGrid(
            ...
            items(movieLisState.popularMovieList.size){index ->
                MovieItem(
                    movie = movieLisState.popularMovieList[index],
                    navHostController = navController
                )
                Spacer(modifier = Modifier.height(16.dp))

                if(index >= movieLisState.popularMovieList.size - 1 && !movieLisState.isLoading){
